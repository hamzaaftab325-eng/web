...
interface CareGuide {
  ...
  excerpt: string; sortOrder: number; isActive: boolean;
}
...
export default function AdminCareGuides() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/admin/content/care-guides", {
        method: "POST",
      ...
      setGuides([...guides, data.careGuide]);
      setShowForm(false);
    ...
    <div>
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-gradient-card-warm border border-hairline-cream rounded-sm p-6 mb-6">
          <h2 className="t-headline-sm c-ink mb-4">New Care Guide</h2>
