...
import { Users, Plus, Trash2, Star, Pencil, ArrowUp, ArrowDown } from "lucide-react";
...
export default function AdminTestimonials() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const url = isEditing ? `/api/admin/content/testimonials/${editingId}` : "/api/admin/content/testimonials";
      ...
      const returned = data.testimonial;
  ...
  const moveItem = async (id: string, direction: "up" | "down") => {
    ...
    await Promise.all([
      fetch(`/api/admin/content/testimonials/${a.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sortOrder: b.sortOrder }) }),
      fetch(`/api/admin/content/testimonials/${b.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sortOrder: a.sortOrder }) }),
    ]);
    ...
    <div>
      <div className="mb-10 relative">
        ...
        <div className="relative flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          ...
          <button onClick={() => { if (isEditing) cancelForm(); else { resetForm(); setShowForm(!showForm); } }} className="group inline-flex items-center gap-2 bg-ink c-paper t-label-caps px-6 py-3.5 rounded-sm hover:bg-gold-deep transition-colors flex-shrink-0">
            ...
            {showForm ? (isEditing ? "Cancel Edit" : "Cancel") : "Add Testimonial"}
          </button>
        </div>
      </div>
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-gradient-card-warm border border-hairline-cream rounded-sm p-6 mb-6">
          <h2 className="t-headline-sm c-ink mb-4">{isEditing ? "Edit Testimonial" : "New Testimonial"}</h2>
          ...
          <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-full flex items-center gap-3">
              <button type="submit" disabled={saving} className="inline-flex items-center gap-2 bg-ink c-paper t-label-caps px-6 py-3 rounded-sm hover:bg-gold-deep transition-colors disabled:opacity-50">
                {saving ? (isEditing ? "Saving..." : "Creating...") : (isEditing ? "Save Changes" : "Create Testimonial")}
              </button>
