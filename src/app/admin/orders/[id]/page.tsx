...
import { ArrowLeft, MapPin, Package, ChevronRight, Download, User, AlertTriangle, Check } from "lucide-react";
              ...
              {order.items.map(item => (
                <div key={item.id} className="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
                  <div className="relative w-12 h-12 bg-cream border border-hairline-cream overflow-hidden flex-shrink-0 rounded-sm">
                    {item.productImage ? (// eslint-disable-next-line @next/next/no-img-element
          ...
          <section className="bg-gradient-card-warm border border-hairline-cream rounded-sm p-6">
            ...
            <div className="mt-4 pt-4 border-t border-hairline-cream space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <input type="text" placeholder="Tracking number" defaultValue={order.trackingNumber ?? ""} onBlur={(e) => { const val = e.target.value; if (val !== (order.trackingNumber ?? "")) { fetch(`/api/admin/orders/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ trackingNumber: val || null }) }).then(() => setOrder({ ...order, trackingNumber: val || undefined })); } }} className={inputCls} />
                <input type="text" placeholder="Carrier (e.g. TCS, DHL)" defaultValue={order.carrier ?? ""} onBlur={(e) => { const val = e.target.value; if (val !== (order.carrier ?? "")) { fetch(`/api/admin/orders/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ carrier: val || null }) }).then(() => setOrder({ ...order, carrier: val || undefined })); } }} className={inputCls} />
              </div>
              ...
              <>
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-ink to-ink/80 flex items-center justify-center flex-shrink-0">
                    <span className="t-label-caps c-paper">{order.customer.name.split(" ").map(n => n[0]).join("").slice(0, 2)}</span>
                  </div>
                  <div>
