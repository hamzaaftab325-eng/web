...
function buildLowStockEmail(params: {
  ...
  const outOfStockRows = outOfStock.map((p) => [
    `<a href="https://aura-living-1.vercel.app/product/${p.slug}" style="color:#1a1714;text-decoration:none;font-weight:500;">${p.name}</a><br><span style="font-size:11px;color:#718096;">${p.category?.name ?? "Uncategorized"} · ₨${Number(p.price).toLocaleString()}</span>`,
  ...
  const content = `
    ...
    <div style="text-align:center;">
      <a href="https://aura-living-1.vercel.app/admin/products" class="button">Manage Products</a>
    </div>
  `;
