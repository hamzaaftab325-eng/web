...
<title>Invoice ${order.orderNumber} — Aura Living</title>
<style>
...
<div class="invoice">
  <div class="header">
    ...
    <div class="invoice-meta">
      ...
      Date: ${order.createdAt.toISOString().split("T")[0]}<br>
  ...
  <div class="section">
    <h3>Bill To</h3>
    <p>${order.user ? `${order.user.firstName} ${order.user.lastName}` : addr?.firstName ?? ""} ${addr?.lastName ?? ""}<br>
    ${addr?.street ?? ""}<br>
  ...
  <table>
    ...
    <tbody>
      ${order.items.map(item => `<tr><td>${item.productName}</td><td class="right">${item.quantity}</td><td class="right">${formatPrice(Number(item.price))}</td><td class="right">${formatPrice(Number(item.price) * item.quantity)}</td></tr>`).join("")}
    </tbody>
  </table>
  <div class="totals">
    <div class="row"><span>Subtotal</span><span>${formatPrice(Number(order.subtotal))}</span></div>
    <div class="row"><span>Shipping</span><span>${Number(order.shippingCost) === 0 ? "Free" : formatPrice(Number(order.shippingCost))}</span></div>
    ${Number(order.discount) > 0 ? `<div class="row"><span>Discount</span><span>−${formatPrice(Number(order.discount))}</span></div>` : ""}
    <div class="row total"><span>Total</span><span>${formatPrice(Number(order.total))}</span></div>
  </div>
  <div class="footer">
