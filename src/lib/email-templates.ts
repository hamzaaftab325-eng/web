...
export function orderConfirmationEmail(
  ...
  customerName: string,
  breakdown?: { subtotal: string; discount?: string; shipping: string; tax: string }
  ...
  const breakdownHtml = breakdown ? `
    <div class="totals">
      <div class="row"><span>Subtotal</span><span>${breakdown.subtotal}</span></div>
      ${breakdown.discount ? `<div class="row"><span>Discount</span><span style="color:#D4AF37;">${breakdown.discount}</span></div>` : ""}
      <div class="row"><span>Shipping</span><span>${breakdown.shipping}</span></div>
      <div class="row"><span>Tax</span><span>${breakdown.tax}</span></div>
    </div>` : "";
  return {
