...
export async function middleware(request: NextRequest) {
  ...
  if (accessToken) {
    try { const p = await verifyTokenEdge(accessToken); isAuthenticated = true; userRole = p.role; } catch {}
  }
  if (!isAuthenticated) {
    const refreshToken = getRefreshToken(request);
    if (refreshToken) { try { const p = await verifyTokenEdge(refreshToken); isAuthenticated = true; userRole = p.role; } catch {} }
  }
