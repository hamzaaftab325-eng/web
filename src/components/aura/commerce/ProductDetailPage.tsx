...
export function ProductDetailPage({ product, onBack }: ProductDetailPageProps) {
  ...
  const onAddToCart = async () => {
    ...
    if (giftWrap) {
      // Gift wrap is a $8 service line — add a synthetic product line so
      // the cart reflects the surcharge. We use the product's slug to
      // namespace the gift-wrap line.
      addToCart(
        {
          id: `${product.id}-giftwrap`,
          slug: `${product.slug}-giftwrap`,
          name: `Gift wrap — ${product.name}`,
  ...
  const unitPrice = product.price + (giftWrap ? GIFT_WRAP_PRICE : 0);
                ...
                <div className="flex items-stretch gap-3 mb-3">
                  ...
                  <div className="inline-flex items-center border border-hairline rounded-sm bg-paper">
                    ...
                    <button
                      ...
                      className="h-12 w-12 flex items-center justify-center c-ink hover:c-gold-deep transition-colors"
                    >
                  ...
                  <button
                    ...
                    className={cn(
                      "flex-1 h-12 px-6 flex items-center justify-center gap-2 t-label-caps transition-all relative overflow-hidden rounded-sm",
                      addingState === "idle"
