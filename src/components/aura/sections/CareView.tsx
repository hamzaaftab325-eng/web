...
interface CareViewProps {
  initialGuides?: Awaited<ReturnType<typeof import("@/lib/services/content.service").getCareGuides>>;
}

export function CareView({ initialGuides }: CareViewProps = {}) {
  const { data: careGuides = initialGuides ?? [] } = useCareGuides() as { data: typeof initialGuides };
      ...
      <section className="pb-12 md:pb-16">
        <div className="container-aura max-w-3xl">
          <RevealOnScroll stagger={0.06} className="space-y-6">
            {(() => {
              // Normalize body to an array of blocks (same pattern as JournalArticleView)
              // Cast to unknown first — the type says array but at runtime can be a string.
              const rawBody = guide.body as unknown;
              let blocks: { type: "paragraph" | "heading" | "list"; text?: string; items?: string[] }[] = [];

              if (typeof rawBody === "string") {
                const paragraphs = rawBody.split(/\n\n+/).filter((p) => p.trim().length > 0);
                blocks = paragraphs.map((text) => ({ type: "paragraph", text }));
              } else if (Array.isArray(rawBody)) {
                blocks = rawBody as { type: "paragraph" | "heading" | "list"; text?: string; items?: string[] }[];
