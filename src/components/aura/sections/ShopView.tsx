...
interface ShopViewProps {
  /** Server-side fetched products. When provided, no spinner on initial load. */
  initialProducts?: Awaited<ReturnType<typeof import("@/lib/services/product.service").getAll>>["products"];
...
export function ShopView({ initialProducts, initialCategories, initialCollections }: ShopViewProps = {}) {
  // When initialProducts is provided (server-side), skip the client-side
  // query entirely — no wasted network request. Only fetch if no SSR data.
  const productsQuery = useProducts({ limit: 100 }, { enabled: !initialProducts });
  const productData = productsQuery.data;
  const products = useMemo(
    () => (initialProducts ?? (productData?.products ?? [])) as Product[],
    [initialProducts, productData]
  );
  const isLoading = !initialProducts && productsQuery.isLoading;
  const isError = !initialProducts && productsQuery.isError;
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result;
  }, [products, activeCategory, filters, sort, collection]);
            ...
            <div className="flex-1 min-w-0">
              ...
              {isLoading && (
                <div className="flex flex-col items-center justify-center py-32">
                  ...
                  <p className="mt-6 t-label-caps c-ink-faint">Loading products…</p>
                </div>
              )}
              ...
              {isError && !isLoading && (
                <div className="text-center py-32">
                  <p className="t-headline-sm c-ink mb-3">Couldn&apos;t load products</p>
                  <p className="t-body c-ink-muted mb-6">Something went wrong. Please try again.</p>
                  <button
              ...
              {/* Grid — empty state (loaded but no products match) */}
              {!isLoading && !isError && filteredProducts.length === 0 && (
                <div className="text-center py-32">
                  <p className="t-headline-sm c-ink mb-3">No products found</p>
                  <p className="t-body c-ink-muted mb-6">Try adjusting your filters or browse the full catalogue.</p>
                  <button
                    onClick={() => { clearFilters(); router.push("/shop"); }}
                    className="inline-flex items-center gap-2 bg-ink c-paper t-label-caps px-6 py-3 rounded-sm hover:bg-gold-deep transition-colors"
                  >
                    Clear Filters
                  </button>
