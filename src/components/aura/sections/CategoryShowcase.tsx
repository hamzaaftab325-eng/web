...
export function CategoryShowcase({ initialCategories, initialProducts }: CategoryShowcaseProps = {}) {
  const { data: categories = initialCategories ?? [] } = useCategories();
  const { data: productData } = useProducts();
  const products = initialProducts ?? (productData?.products ?? []);
  const setCategory = useUIStore((s) => s.setCategory);

  const cards = (categories as Array<{ slug: string; name: string; description?: string | null; heroImage?: string | null; productCount?: number }>).map((c) => ({
    ...c,
    ...
    <section className="section-stack bg-canvas">
      <div className="container-aura">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12 md:mb-16">
          <div>
            <p className="t-label-caps c-gold-deep mb-3 flex items-center gap-2"><span className="w-6 h-px bg-gold" aria-hidden />Categories</p>
            <TextBlurReveal
              as="h2"
