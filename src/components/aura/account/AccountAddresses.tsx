...
export function AccountAddresses() {
  ...
  // Fetch addresses from the API on mount.
  // Map API field names (province/postalCode) to UI field names (state/zip).
  useEffect(() => {
    fetch("/api/user/addresses")
      .then((r) => (r.ok ? r.json() : []))
      .then((data) => {
        const raw = Array.isArray(data) ? data : [];
        const mapped: Address[] = raw.map((a: Record<string, unknown>) => ({
          id: String(a.id ?? ""),
          ...
          zip: String(a.postalCode ?? a.zip ?? ""),
          country: String(a.country ?? "Pakistan"),
  ...
  const saveAddress = async () => {
    ...
    // Map UI field names to API field names: state → province, zip → postalCode
    const payload = {
    ...
    try {
      if (editingId) {
        // PUT to update existing address
        ...
        setAddresses((list) => {
          let next = list.map((a) => (a.id === editingId ? updated : a));
          if (form.isDefault) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/user/addresses/${targetId}`, { method: "DELETE" });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.error ?? "Failed to delete");
      }
      setAddresses((list) => {
        const next = list.filter((a) => a.id !== targetId);
